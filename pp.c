...
char *dir_path = "/home/iqbalhumam73/Downloads";
char *log_path = "/home/iqbalhumam73/fs.log";
...
static int xmp_getattr(const char *path, struct stat *stbuf){
    ...
    char temp[1000];
    strcpy(temp, path);
    if(strncmp(path, "/encv1_", 6) == 0) decryptV1(temp);
    char fpath[1000];
...
static int xmp_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi){
    ...
    if (strcmp(path, "/") == 0){
        ...
        strcpy(tmp, path);
        if(strncmp(path, "/encv1_", 6) == 0) decryptV1(tmp);
    ...
    while ((de = readdir(dp)) != NULL){
        ...
        if (strncmp(path, "/encv1_", 6) == 0) encryptV1(temp);
...
static int xmp_rename(const char *from, const char *to){
    ...
    sprintf(ffrom, "%s", from);
        ...
        if(strncmp(from, "/encv1_", 6) == 0) decryptV1(temp);
    ...
    if (strcmp(to, "/") == 0){
        ...
        if(strncmp(to, "/encv1_", 6) == 0) decryptV1(temp);
